...
parser.add_option("-P", "--maxNPF", dest='maxNPF', help='max number of particle', default=100, type='int')
...
X = np.zeros(shape=(maxEntries, opt.maxNPF, nFeatures), dtype=float, order='F')
# recoil estimators
Y = np.zeros(shape=(maxEntries, 2), dtype=float, order='F')

pt = to_np_array(tree['L1PuppiCands_pt'], maxN=opt.maxNPF)
eta = to_np_array(tree['L1PuppiCands_eta'], maxN=opt.maxNPF)
phi = to_np_array(tree['L1PuppiCands_phi'], maxN=opt.maxNPF)
pdgid = to_np_array(tree['L1PuppiCands_pdgId'], maxN=opt.maxNPF, pad=-999)
charge = to_np_array(tree['L1PuppiCands_charge'], maxN=opt.maxNPF, pad=-999)
puppiw = to_np_array(tree['L1PuppiCands_puppiWeight'], maxN=opt.maxNPF)
